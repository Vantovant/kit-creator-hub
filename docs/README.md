# Suite Bridge — MLM Course Platform Spoke Bundle

**Spoke:** MLM Course Platform  
**Domain:** dashboard.onlinecourseformlm.com  
**APP_KEY:** `mlm_course`  
...
Paste the contents of **`suite-bridge-spoke.ts`** from this bundle. `APP_KEY` is already set to `mlm_course` — do not change it.
...
- **Value:** the exact same random string that was pasted into VantoOS under `SUITE_BRIDGE_SECRET_MLM_COURSE`.
...
```json
{ "ok": true, "app": "mlm_course", "kind": "pong", "ts": <epoch_ms> }
```
