...
//   - list_inbox_messages and list_inbox_accounts are read-only.
//   - sync_gmail_inbox (added 2026-08-13) triggers a real Gmail fetch across
...
function buildServer() {
  ...
  server.registerTool(
    "list_inbox_accounts",
    {
      title: "List connected Gmail/Reply Inbox accounts",
      description:
        "List the connected Gmail accounts (mailboxes) feeding the Reply Inbox, " +
        "with status, last_sync_at, and any sync_error. Use this to see all " +
        "connected mailboxes and get an account_id to pass to sync_gmail_inbox. " +
        "Read-only.",
      inputSchema: {
        include_inactive: z.boolean().optional().describe("Defaults to false (inactive/disconnected accounts hidden)"),
      },
    },
    async (args) => {
      const data = await callBridge("list_inbox_accounts", args);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    "sync_gmail_inbox",
    {
      title: "Sync the Reply Inbox from Gmail",
      description:
    ...
    async (args) => {
      const data = await callBridge("sync_gmail_inbox", args);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    "list_inbox_messages",
    {
      ...
      description:
        ...
        "Gmail sync — reads existing rows only; use sync_gmail_inbox first for " +
        "fresh data). Scoped to the configured owner account. Read-only.",
      inputSchema: {
