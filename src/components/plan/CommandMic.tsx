...
import { Zap, MicOff, Loader2, Check, X, ListTodo, Bell, CalendarDays } from "lucide-react";
...
export function CommandMic({ onConfirm, variant = "header" }: {
  onConfirm: (intent: ParsedIntent) => Promise<void>;
  variant?: "header" | "fab" | "commandbar";
}) {
  ...
  return (
    <div className="relative">
      {/* Header / CommandBar button */}
      {(variant === "header" || variant === "commandbar") && (
        <button
          onClick={startListening}
          disabled={listening}
          title="Voice Command (creates task/reminder/meeting after confirmation)"
          className={`p-2 rounded-lg transition-colors ${listening ? "bg-destructive/10 text-destructive animate-pulse" : "bg-amber-500/10 text-amber-600 hover:bg-amber-500/20"}`}
        >
          {listening ? <MicOff className="w-4 h-4" /> : <Zap className="w-4 h-4" />}
        </button>
      )}
      ...
      {variant === "fab" && (
        <button
          ...
          disabled={listening}
          title="Voice Command (creates task/reminder/meeting after confirmation)"
          className={`fixed bottom-4 right-4 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-colors ${
            listening ? "bg-destructive text-destructive-foreground animate-pulse" : "bg-amber-500 text-white"
          }`}
        >
          {listening ? <MicOff className="w-6 h-6" /> : <Zap className="w-6 h-6" />}
        </button>
      )}
    </div>
  );
