...
-- Add CRM fields to prospects table (all optional to support both standard and CRM imports)
ALTER TABLE public.prospects
  ...
  ADD COLUMN IF NOT EXISTS go_status TEXT,
  ADD COLUMN IF NOT EXISTS associate_status TEXT,
  ...
  ADD COLUMN IF NOT EXISTS date_captured TIMESTAMP WITH TIME ZONE,
  ADD COLUMN IF NOT EXISTS interest_level TEXT,
  ADD COLUMN IF NOT EXISTS focus_area TEXT,
  ADD COLUMN IF NOT EXISTS lead_path TEXT,
...
-- Add index on phone_normalized for dedup matching
CREATE INDEX IF NOT EXISTS idx_prospects_phone_normalized ON public.prospects(phone_normalized) WHERE phone_normalized IS NOT NULL;

-- Add index on email_normalized for dedup matching
CREATE INDEX IF NOT EXISTS idx_prospects_email_normalized ON public.prospects(email_normalized) WHERE email_normalized IS NOT NULL;

-- Backfill email_normalized from existing emails
UPDATE public.prospects SET email_normalized = LOWER(TRIM(email)) WHERE email_normalized IS NULL;

-- Update get_segment_prospects to support new CRM fields
...
DECLARE
  ...
  final_where TEXT;
  base_query TEXT;
  text_fields TEXT[] := ARRAY['email', 'first_name', 'full_name', 'source', 'phone_number', 'phone_normalized', 'email_normalized', 'lead_type', 'registration_status', 'go_status', 'associate_status', 'communication_status', 'lead_temperature', 'city', 'province', 'state', 'country', 'interest_level', 'focus_area', 'lead_path', 'sponsor_name', 'assigned_to', 'action_taken', 'next_action', 'aplgo_id', 'additional_notes'];
  date_fields TEXT[] := ARRAY['created_at', 'last_activity_at', 'date_captured', 'meeting_time'];
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF field_val = ANY(text_fields) THEN
        CASE op_val
      ...
      ELSIF field_val = ANY(date_fields) THEN
        CASE op_val
